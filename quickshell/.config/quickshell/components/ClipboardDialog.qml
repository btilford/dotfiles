...
// singleton. v1 (this pass): search + flat list + Enter-to-copy. Preview pane,
// tree mode, actions/pin/delete parity land in later passes.
PanelWindow {
    id: root
    ...
    Socket {
        ...
        parser: SplitParser {
            ...
            onRead: function (line) {
                const kind = root.reqQueue.length ? root.reqQueue.shift() : "";
                let msg;
                try {
                    msg = JSON.parse(line);
                } catch (e) {
    ...
    function sendReq(kind, req) {
        root.reqQueue.push(kind);
