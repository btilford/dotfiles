...
| `lefthook.yml` pre-commit | `git commit --no-verify`, or not running `mise run hooks` |
...
built from the same `gitleaks` config the rest of the repo uses.
...
install -m 0755 pre-receive.d/gitleaks "$HOOKS_DIR/pre-receive.d/gitleaks"
gitleaks version   # must be on PATH for the git user
```
...
- **Fails closed when gitleaks is missing.** A security gate that disables itself
  when its binary disappears is worse than no gate, because the green push then
  reads as proof. The deliberate override is
  `GITLEAKS_PRE_RECEIVE_DISABLE=1` in the server environment — an admin action,
...
- **`--redact`.** Output reaches the pusher's terminal and the server log. File
  ...
  more places.
- **No repo config.** It runs with gitleaks' default rule set, because the hook
  is instance-wide and cannot assume any particular repo's `.gitleaks.toml`.
