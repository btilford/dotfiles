...
$env.config = {
    ...
    table: {
        mode: rounded #, compact, compact_double, light, thin, with_love, rounded, reinforced, heavy, none, other
    ...
    filesize: {
       ...
       # unit: binary #auto # b, kb, kib, mb, mib, gb, gib, tb, tib, pb, pib, eb, eib, auto
    }
...
# The four files below are gitignored — host-specific, and regenerated by env.nu on
# every start. env.nu runs to completion before this file is parsed, so they are
# guaranteed to exist by the time these lines resolve; env.nu stubs any whose tool is
# missing for the same reason.
#
# Do not "defend" these with `if ... path exists`: `source` resolves at parse time, so
# a missing file is a parse error that kills the shell before the guard ever runs. The
# guard reads as protection and provides none. env.nu creating the file is the only
# thing that actually protects a cold start.
