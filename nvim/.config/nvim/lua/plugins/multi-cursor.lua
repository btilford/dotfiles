return {
	{
		...
		config = function()
			...
			vim.keymap.set({ "n", "v" }, "gms", mcos.opkeymapfunc, { expr = true, desc = "Multi-cursor: Add cursors by motion" })
			vim.keymap.set({ "n" }, "gmss", mcos.bufkeymapfunc, { desc = "Multi-cursor: Add cursors to buffer" })
		end,
	},
	{
		"jake-stewart/multicursor.nvim",
